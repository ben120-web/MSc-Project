function generatingNoisyEcgDatabase(noiseSignalPath, ecgSignalPath, ecgFs, maxNosieSections, SNR, numberOfGeneratedNoisySignals, googleDriveFolderIDClean, snrFolderIDs)
...
%
% Syntax: generatingNoisyEcgDatabase(noiseSignalPath, ecgSignalPath, ecgFs, maxNosieSections, SNR, numberOfGeneratedNoisySignals, googleDriveFolderIDClean, snrFolderIDs)
%
% Inputs:
...
for iEcgFile = 1:nEcgFiles
    ...
    % Call function to convert and save each signal to a readable format for deep learning models and upload to Google Drive.
...
cleanFilePath = fullfile(tempdir, strcat(fileName, '.h5'));
...
for iSection = 1:height(DataTable)
    ...
    for iSNR = 1:NUM_OF_SNR
        ...
        for iCopy = 1:numel(sectionNoise)
            ...
            outputFolderNoisySub = tempdir;

            % Let's create a filename based on the type of signal.
            outputFilename = fullfile(outputFolderNoisySub, strcat(erase(fileName, '_cleanSignal'), '-', noiseType, '-', num2str(sectionNumber), '.h5'));
...
%------------- BEGIN CODE --------------
    ...
    fid = fopen('/path/to/your/credentials.json', 'r'); % Update this path
    raw = fread(fid, inf);
    str = char(raw');
    fclose(fid);
    ...
    refreshToken = 'YOUR_REFRESH_TOKEN'; % Use the refresh token you obtained earlier
    ...
    % Upload the file
    response = webwrite(apiEndpoint, body, options);
